...
class DatabaseHandler:
    ...
    async def reset_current_score(self, user_id: int):
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute('''INSERT INTO quiz_state (user_id, current_score) 
                             VALUES (?, 0) 
                             ON CONFLICT(user_id) DO UPDATE SET current_score = 0''',
                             (user_id,))
            await db.commit()

    async def increment_current_score(self, user_id: int):
        async with aiosqlite.connect(self.db_path) as db:
            await db.execute('''INSERT INTO quiz_state (user_id, current_score) 
                             VALUES (?, 1) 
                             ON CONFLICT(user_id) DO UPDATE SET current_score = current_score + 1''',
                             (user_id,))
            await db.commit()

    async def get_current_score(self, user_id: int) -> int:
        async with aiosqlite.connect(self.db_path) as db:
